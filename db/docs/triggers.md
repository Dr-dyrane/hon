...
| `set_customer_records_updated_at` | `app.customer_records` | `before` | `update` | `app.set_updated_at()` | Keeps CRM notes, tags, and support-state edits timestamped. | `0021__customer_crm_records.sql` | Covers admin-only customer metadata updates. |
...
| `audit_customer_records_change` | `app.customer_records` | `after` | `insert, update, delete` | `audit.log_row_change()` | Records CRM notes, tags, and support-state mutations in the audit ledger. | `0021__customer_crm_records.sql` | Uses `customer_key` as the tracked record key. |
