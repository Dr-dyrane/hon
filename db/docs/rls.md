...
| `app.customer_records` | `customer_records_admin_all` | `all` | `admin` | Only admin can read and write CRM notes, tags, and support-state records. | `app.has_role` | `0021__customer_crm_records.sql` | Keeps customer metadata private to operations while still enforcing row security on the table. |
